...
$(function(){
	...
	$("#editar_reserva").click(function(){$("#flogin").toggle('low');});	
	...
	$("#selectorComensals input[value=grups]").click(function(){window.location.href="form.php";return false;});
	
	$("#cb_factura").click(function(){
		if ($("#cb_factura:checked").val()=="on") $(".factura").show();
		else $(".factura").hide();	
	});
...
$(".ui-accordion-content").css("overflow","scroll");
	...
	$("#help").dialog("open");
		...
		{
                    comportamentClient();
			//$(".fr-seccio-client").slideDown("slow",function(){seccio("fr-seccio-client");});
...
function comportamentDia()
{
	...
	$("#calendari").change(function(){
		...
		{
			$(".fr-seccio-hora").slideDown("slow",function(){seccio("fr-seccio-hora");});
		}
...
function updateResum()
{
	...
	//$("#resum-comanda").html( menua+ (menuj?" / ":"") + menuj + (menun?" / ":"") + menun);
	$("#resum-data").html($("#calendari").val());
...
function updateMenus(){
	...
	if (excepcio){
		 ...
		 * ADUULTS
		 */
		//$("#carta a[v=0]").parent().hide(); //menu 1
		$("#carta a[v=1]").parent().hide();
		//$("#carta a[v=2]").parent().hide(); // menu 2
		$("#carta a[v=3]").parent().hide();
		//$("#carta a[v=4]").parent().hide(); // menu n3
		 ...
		 * JUNIORS
		 */
		//$("#carta2 a[v=junior]").parent().hide();
		$("#carta2 a[v=jr_comu]").parent().hide(); 
		$("#carta2 a[v=jr_casa]").parent().hide();
		/*
		 * NENS
		 */
		//$("#carta3 a[v=infantil]").parent().hide();
...
function llistanegra(date)
{
	...
	t = LLISTA_NEGRA[m];
	if (t) for (var i in t) if (t[i] == d) bloqDia=true;

	// COMPROVA LLISTA NITS
	t = LLISTA_NITS_NEGRA[m];
	if (t && !bloqNit) for (var i in t) if (t[i] == d) bloqNit=true;
...
function seccio(selector_seccio){
	if (selector_seccio) SECCIO=selector_seccio;
