var browser_malo=(jQuery.browser.msie && jQuery.browser.version>'7');
...
function recargaHores()
{
	...
	$.post(GESTOR+"?a=horesDisponibles&b="+$("#calendari").val()+"&c="+comensals+"&d="+$("input[name='selectorCotxets']:checked").val()+"&e="+accesibilidad+"&f="+IDR, function(dades) {
		...
		$("input[value='"+hora+"']").attr("checked",true).button("refresh");
...
function updateCarta(menu)
{
	...
	else
	{
		...
		taula+='<tr style="background:#eee;"><td class="resum-carta-nom"><h3>TOTAL</h3></td><td class="resum-carta-preu" style="text-alig:right"><h3>'+total+'€ <br/>(IVA '+l("inclòs")+')</h3></td><td></td></tr>';
	}
...
function validacio()
{
	$("#form-reserves").validate({
		...
		messages: {
			totalComensals: {
				required: l("TOTAL!!!"),
				min: l("Selecciona, coma a mínim, dos adults"),
