...
class DBTable extends Gestor
{
	...
	public function DBTable($table_query)
	{
		...
		if (!empty($order) && substr($order,0,8)!="ORDER BY") $order="ORDER BY ".$order;
		$order = trim($order);$order=" ".$order;
	...
	public function controlsTaula($fila,$columna)
	{
		...
		if ($ui=="idR") 
			...
			//return '<a href="'.$_SERVER['PHP_SELF'].'?'.$columna.'='.$fila[$columna].'">'.$fila[$columna].'</a>';
