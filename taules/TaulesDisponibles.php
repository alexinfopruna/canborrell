...
class TaulesDisponibles extends Gestor
{
	...
	public function taules()
	{
		...
		if (!$this->restaurantObert()) return $this->addError(23);
...
/**/
		...
		//
		if ($this->reserva_id 
		&& $this->data==$this->reserva->data 
		&& $this->torn==$this->reserva->torn 
		&& ($this->reserva->persones==$persones 
			|| ($this->reserva->taula->persones>$persones && $this->reserva->taula->plena==false && $this->taulesGrans))
		&& ($this->reserva->cotxets==$cotxets || 
		...
		else usort($this->arResultatTaula,array('TaulesDisponibles','ordenaPunts'));
...
/************************************************************************************************************************/
	function ordenaPunts($a,$b){return $a->punts<$b->punts;}
...
/************************************************************************************************************************/
	private function restaurantObert()
	{
		...
		if ($this->torn==3 && !$this->nitsObert[$ds] && !$this->excepcionsNits($dia)) return $this->addError(24); // TORN NIT DX,DJ,DG
		...
		if ($ds>0 && $ds<3 && !$this->diaDinsLlista($this->llista_dies_blanca)) return $this->addError(25);	//DL, DM en llista blanca
		return true;		
	}
/************************************************************************************************************************/	
	private function excepcionsNits($dia){
		...
		if ($ds==5 && $data[1]==12 && $data[2]>10 && $data[2]<25) return true;
...
/************************************************************************************************************************/	
	...
	{
		if (!$fitxer || !file_exists($fitxer)) return false;
		...
		{
			if ($dia==$dies[$mes-1][$i]) return true;
			$i++;
		}
		return false;
	}
...
/************************************************************************************************************************/
	...
	{
		if (!$this->arResultatTaula) return;
		...
		foreach ($this->arResultatTaula as $k => $taula)
		{
			if ($taula->bloquejada($bloquejats)) 
			{
				...
				if ($del)array_splice($this->arResultatTaula,$pos,1);
				if ($del)array_splice($this->arResultat,$pos,1);
...
/************************************************************************************************************************/
	...
	{	
		...
		if (($this->totalTorn+$this->persones) > $this->maxTorn) return false;
		return true;
	}
...
/************************************************************************************************************************/
	...
	public function loadReserva($idr)
	{
		...
		if (!$this->reserva->id) return false;
		return $this->reserva;
	}
...
/************************************************************************************************************************/
	public function recupera_hores($mostraDisableds=false)
	{
		...
		//		Cal mirar hores bloq. (base+torn a la taula que toqui)
		////////////////////////////////////////////////////////////////////////////
		elseif (!$taules || !sizeof($taules)) return $this->addError(29);//NO TROBA TAULA return "Ple o tancat";
		////////////////////////////////////////////////////////////////////////////
		...
		elseif ($this->taula_id)
		{
			...
			if (!$trobat) return $this->addError(30);
		}
		$data=$this->data;
		$torn=$this->torn;
...
ORDER BY  `estat_hores_hora` ASC ";      
		...
		{			
			...
			if ($row['estat_hores_hora'] == $this->hora) $checked = 'checked="checked"'; // CHECKED
			else 
			{
			...
			if ($table!=$this->defaultTableHores) $row['estat_hores_max']=$this->recuperaMaximHora($row['estat_hores_hora']);
...
/**/
	...
	{
		if (!$ro=$this->restaurantObert()) return $this->addError(23);
...
****************************************/		
		...
		while ($row = mysql_fetch_array($Result1))
		{
			...
			if ($row['estat_hores_hora'] == $valor) $checked = 'checked="checked"';			
			else 
			{
				if (!$row['estat_hores_actiu']) continue;
				$checked = "";
...
/************************************************************************************************************************/	
	...
	{	
			foreach($this->arResultatTaula as $k => $v) if ($v->id == $taula_id) return $taula_id;
		return false;
...
/************************************************************************************************************************/	
	...
	{
		if ($this->arResultatTaula && count($this->arResultatTaula))	return $this->arResultatTaula;
...
/************************************************************************************************************************/
	...
	{
		if (!isset($this->arHores) || !count($this->arHores)) $this->recupera_hores();
		if (!$this->arHores) return false;
...
/*********************************************************************************/
	public function dump()
	{
		...
		{
			if (count($this->arError)) foreach($this->arError as $k=>$v) $table.= "ERROR!!!!!!!!!!!!! ---> TORN ".$this->torn." >>> err $v ".$this->arTxtError[$v]."<br/>";
		}		
		...
		";
		foreach($this->nitsObert as $k=>$v) $estat.="NIT $k: $v <br/>";
...
/*********************************************************************************/
/*********************************************************************************/
if (!defined('LLISTA_DIES_NEGRA')) define("LLISTA_DIES_NEGRA",INC_FILE_PATH."llista_dies_negra_online.txt");
if (!defined('LLISTA_NITS_NEGRA')) define("LLISTA_NITS_NEGRA",INC_FILE_PATH."llista_dies_negra_online.txt");
...
if (!empty($_GET['d']) && !empty($_GET['t']) && !empty($_GET['p']) ) //PER FER-LI TEST
{
	...
	$g->persones=$_GET['p'];
	$g->cotxets=$_GET['c'];
	if (isset($_GET['a'])) $g->accesible=$_GET['a'];
	...
	//RESERVA
	if(isset($_GET['r'])) $g->loadReserva($_GET['r']);
	if(isset($_GET['m']))$g->loadTaula($_GET['m']);
	$g->torn=1;
	echo "<br/>
			-------------------------------------------------------------------<br/>
			-------------------------------------------------------------------<br/>
