...
function procesaResposta(resposta_raw){
	 ...
	 * MISSATGE DIA
	 */
	if (resposta.missatge_dia) $(".missatge_dia").html(resposta.missatge_dia);
	...
	if (resposta.hores) {
		...
		$( "#inserta_res_radio input",FORMULARI_INSERTA_RES ).change(function(){max_comensals=$(this).attr("maxc");});
...
function procesaAccordions(reserves){
	...
	$(".fr",TABS).mouseover(function(){taulaSel=$(this).attr("taula");FLASH.seleccionaTaula(taulaSel);});
	$(".fr",TABS).mouseout(function(){taulaSel=$(this).attr("taula");FLASH.seleccionaTaula(0);});
...
function nodeAccordionReserves(node){
	...
	if (permisos>64) bt_delete='<div class="delete reserva ui-state-default ui-corner-all"> \
    <a href="taules.php?del_reserva='+node.id_reserva+'" del="'+node.id_reserva+'">Elimina</a></div>';
	
	if (node.sobret) sobret='<div style="position:relative;left:0" class="ui-icon '+node.sobret+'" title="Observacions del client">00</div>';
	//online='<div></div>'
	if (node.online) online='<div class="online" title="Reserva ONLINE">'+sobret+'</div>';
