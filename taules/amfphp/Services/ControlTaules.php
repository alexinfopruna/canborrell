...
class ControlTaules
{
	...
	function recuperaEstat($data, $torn)
	{
		...
		$tornB=$torn;if ($torn==1) $tornB=2;if ($torn==2) $tornB=1;
		if ($tornB!=$torn) $cercaTornB=" OR estat_taula_data = '$mydata' AND estat_taula_torn = $tornB AND reserva_id>0";
		
		
		$dataBASE = $this->data_BASE;
		if ($data<$this->data_BASE) $data=$this->data_BASE;
		...
		$Result1 = $this->log_mysql_query($query, $canborrell) or die(mysql_error());
		...
		{							
			...
			$taula = new TaulaVO($row['estat_taula_taula_id'],$row['estat_taula_nom'] ,$row['estat_taula_persones'], $row['estat_taula_cotxets'],
								...
								$client,$row['estat_taula_plena'],$row['estat_taula_torn'],$row['observacions'],$online,$row['reserva_info']);
...
/*********************************************************************************************************************************/	
	function guardaEstat($data,$torn,array $taules)
	{
		...
		{	
			$nid=0;
					$taula=$taules[$i];
						if ($taula->x > 100000) $taula->x=-100;
						if ($taula->y > 100000) $taula->y=-100;
					$mydata=$data;
...
//echo $data." ******** ";
	...
	//	die();
		...
		{		
			...
			$Result1 = $this->log_mysql_query($query, $canborrell) or die(mysql_error());
			$lastId=mysql_insert_id($canborrell);
			if ($lastId) $query = "DELETE FROM estat_menjador WHERE estat_menjador_menjador_id=$menjador AND estat_menjador_id<>$lastId AND estat_menjador_data='$data' AND  estat_menjador_torn='1'";
			$Result1 = $this->log_mysql_query($query, $canborrell) or die(mysql_error());
			...
			$Result1 = $this->log_mysql_query($query, $canborrell) or die(mysql_error());
			$lastId=mysql_insert_id($canborrell);
			if ($lastId) $query = "DELETE FROM estat_menjador WHERE estat_menjador_menjador_id=$menjador AND estat_menjador_id<>$lastId AND estat_menjador_data='$data' AND  estat_menjador_torn='2'";
			$Result1 = $this->log_mysql_query($query, $canborrell) or die(mysql_error());
			...
			$Result1 = $this->log_mysql_query($query, $canborrell) or die(mysql_error());
			$lastId=mysql_insert_id($canborrell);
			if ($lastId) $query = "DELETE FROM estat_menjador WHERE estat_menjador_menjador_id=$menjador AND estat_menjador_id<>$lastId AND estat_menjador_data='$data' AND  estat_menjador_torn='3'";
			$Result1 = $this->log_mysql_query($query, $canborrell) or die(mysql_error());
		}
		else
		{
			...
			$Result1 = $this->log_mysql_query($query, $canborrell) or die(mysql_error());
			$lastId=mysql_insert_id($canborrell);
			if ($lastId) $query = "DELETE FROM estat_menjador WHERE estat_menjador_menjador_id=$menjador AND estat_menjador_id<>$lastId AND estat_menjador_data='$data' AND  estat_menjador_torn='$torn'";
			$Result1 = $this->log_mysql_query($query, $canborrell) or die(mysql_error());
		}
...
/*********************************************************************************************************************************/	
	...
	$this->recuperaSesion($data,$torn);
		...
		if ($data=="1900-01-01") $data=$this->data_BASE;
...
AND reserva_id >0";
			$Result1 = mysql_query($query, $canborrell) or die(mysql_error());
